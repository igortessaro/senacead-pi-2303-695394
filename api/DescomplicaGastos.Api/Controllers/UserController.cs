...
public class UserController : ControllerBase
{
    ...
    public UserController(ILogger<UserController> logger, IUserRespository userRepository, IExpenseRepository expenseRepository)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> GetAsync([FromQuery] string name)
    {
        ...
        if (user is null)
        {
            _logger.LogWarning("{Method}: no user finded with name {Name}", nameof(GetAsync), name);
        }
